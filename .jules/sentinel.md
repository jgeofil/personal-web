...
## 2026-07-22 - Prevent XSS via Nested Mixed Encoding Obfuscation
**Vulnerability:** The `sanitizeUrl` function in `src/lib/security.js` prevented XSS by verifying HTML entities and stripping control characters, and it had a `do...while` loop for iterative URL decoding. However, HTML entity decoding happened *outside* of this iterative loop, allowing attackers to bypass checks using nested encodings (e.g. `&#x25;6aavascript:alert(1)` which becomes `%6aavascript...` after HTML decode but is never URL decoded again).
**Learning:** Attackers often use mixed encodings (URL-encoding HTML entities, or HTML-encoding URL-encoded characters) to bypass sanitization layers that are applied linearly.
**Prevention:** HTML entity decoding and URL decoding must be performed iteratively within the *same* `do...while` loop until the string stops changing. Ensure that the HTML entity decoding step is reached even if `decodeURIComponent` throws an error (e.g. from malformed URI sequences).
